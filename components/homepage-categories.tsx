      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data && data.length > 0) {
            // Show only the first 3 categories on homepage
            setCategories(data.slice(0, 3));
          }
        }
      } catch (error) {
        console.error("Error fetching categories:", error);
        // Set fallback categories with images - matching database structure
        setCategories([
          {
            _id: "1",
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.map((category) => (
            <Card 
              key={category._id} 
              className="bg-[#1a2942] border-gray-800 hover:border-[#10b981]/50 transition-all duration-300 hover:shadow-lg hover:shadow-[#10b981]/10 group h-full flex flex-col"
            >
              <CardContent className="p-0 flex flex-col h-full">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <div className="flex-grow">
                    <h3 className="text-xl font-semibold text-white mb-4 leading-tight group-hover:text-[#10b981] transition-colors duration-300">
                      {category.name}
                    </h3>
                      ...
                      <Button className="w-full bg-gradient-to-r from-[#10b981] to-[#059669] hover:from-[#059669] hover:to-[#047857] text-white font-medium py-3 px-3 rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-[#10b981]/25 transform hover:-translate-y-0.5 group-hover:scale-[1.02] active:scale-[0.98] min-h-[4rem]">
                        <span className="flex items-center justify-center gap-2 text-xs leading-tight">
                          <span className="text-center leading-tight">
                            Explorar<br />
                            <span className="font-normal">{category.name.split(' & ')[0]}</span>
                          </span>
